...
//发布新的成功
var SendNoteEvent = NativeModules.SendNoteEvent;
const mySendNoteEvent= new NativeEventEmitter(SendNoteEvent);
var sendNoteSubscription;
...
export default class Profile extends Component {
    ...
    componentWillMount() {
        ...
        //发布新的成功
        this.sendNoteSubscription = mySendNoteEvent.addListener('sendNoteEvent', (reminder) => {
            this.refreshFunction();
        });
        //头像更改
        this.changHeaderImage =  DeviceEventEmitter.addListener('changHeaderImage' ,(uri)=>{
          this.setState({
              headerImage:uri
          });
        });
            ...
            if (data == '0' || data == 0) {
                ...
                if (isVip == false) {
                    ...
                    this.setState({
                        ...
                        userData: resultData,
                        isVip: true,
                        headerImage:ImgUrl.baseImgUrl + 'user/' + resultData.id + '/avatar'
                    });
    ...
    componentWillUnmount() {
        ...
        this.sendNoteSubscription && this.sendNoteSubscription.remove();
    }
    ...
    _alert(vipState) {
        Util.alertDialog({
            msg: vipState == false ? '成为友邻同学即可享受同学权益' : '成为友邻学员即可收听此课程',
            oneBtn: '立即入学',
            okBtn: '咨询阿树老师',
            cancelBtn: '取消',
    ...
    render() {
        if (!this.state.userData) {
            ...
            let user_id = this.state.userData.id;
              // https://api.youlinyouke.com/v1/user/176459/avatar
              // https://api.youlinyouke.com/v1/user/176459/avatar
            return (
                <View style={{ backgroundColor: '#ffffff', flex: 1 }}>
                    <NavigationView
                    ...
                    <ScrollView>
                        <DetailsHeaderView
                            ...
                            headerImage={this.state.loginStatus ? { isStatic:true, uri: this.state.headerImage } : require('./imgs/nologinImage.png')}
