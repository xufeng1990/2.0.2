...
//
//
//  Created by YLYK on 17/1/12.
//  Copyright © 2017年 . All rights reserved.
//

#import "YLYKPageControl.h"

@interface YLYKPageControl ()
...
@implementation YLYKPageControl
...
- (void)initialize {
    ...
    _numberOfPages = 0;
    _currentPage = 0;
    _pageIndicatorColor = [UIColor colorWithRed:(157/255.0) green:(157/255.0) blue:(157/255.0) alpha:1.0];
    _currentPageIndicatorColor = [UIColor colorWithRed:(178/255.0) green:(29/255.0) blue:(51/255.0) alpha:1.0];
    _indicatorWidth = 3; // 普通宽度
    _indicatorMargin = 5.f; // 间距
    _currentIndicatorWidth = 12.f;// 当前宽度
    _indicatorHeight = 3;
}
...
- (instancetype)initWithFrame:(CGRect)frame indicatorMargin:(CGFloat)margin indicatorWidth:(CGFloat)indicatorWidth currentIndicatorWidth:(CGFloat)currentIndicatorWidth indicatorHeight:(CGFloat)indicatorHeight {
    if (self = [super initWithFrame:frame]) {
        ...
        _currentIndicatorWidth = currentIndicatorWidth;// 当前宽度
        _indicatorHeight = indicatorHeight;
    }
    return self;
}

#pragma mark - setCurrentPageMethod
...
- (void)setCurrentPage:(NSInteger)currentPage sendEvent:(BOOL)sendEvent {
    ...
    for (NSInteger i = 0; i < _numberOfPages; i++) {
        UIImageView * view = self.indicatorViews[i];
        if (i == _currentPage) {
            ...
            view.image = [UIImage imageNamed:@"dot_hight"];
        }else{
            view.image = [UIImage imageNamed:@"dot_default"];
...
- (void)setNumberOfPages:(NSInteger)numberOfPages {
    ...
    if (_numberOfPages != numberOfPages) {
        ...
        for (NSInteger i = 0; i < numberOfPages; i++) {
            if (i == _currentPage) {
                UIImageView * view = [[UIImageView alloc] initWithFrame:CGRectMake(viewX, 0, _currentIndicatorWidth, _indicatorHeight)];
                view.image = [UIImage imageNamed:@"dot_hight"];
                //                view.backgroundColor = _currentPageIndicatorColor;
                ...
                UIImageView * view = [[UIImageView alloc] initWithFrame:CGRectMake(viewX, 0, _indicatorWidth, _indicatorHeight)];
                view.image = [UIImage imageNamed:@"dot_default"];
                //                view.backgroundColor = _pageIndicatorColor;
                view.layer.cornerRadius = _indicatorHeight/2;
                view.layer.masksToBounds = YES;
