...
- (IBAction)sendCodeTouched:(id)sender {
    ...
    if (res1) {
        ...
        if ([self.loginBtn.titleLabel.text isEqualToString:@"确定"]) {
            [YLYKUserServiceModule updateUserMobilephone:USERID :@{@"mobilephone":self.phoneNumberField.text} success:^(id responseObject) {
            ...
            [YLYKTokenServiceModule getTokenByMobilephone:self.phoneNumberField.text success:^(id responseObject) {
               NSDictionary *dict = [NSStringTools getDictionaryWithJsonstring:responseObject];
...
- (IBAction)login:(UIButton *)sender {
    ...
    if ([self phoneNumberIsRight] && [self captchaIsright]) {
        if ([self.loginBtn.titleLabel.text isEqualToString:@"确定"]) {
            [YLYKUserServiceModule updateUserMobilephoneWithCaptcha:USERID :@{@"mobilephone":self.phoneNumberField.text,@"captcha":self.captchaField.text} success:^(id responseObject) {
            ...
            [YLYKTokenServiceModule getTokenByMobilephoneWithCaptcha:self.phoneNumberField.text captcha:self.captchaField.text success:^(id success) {
                //NSLog(@"success");
                [CBLProgressHUD hideLoadingHUDInView:self.view];
