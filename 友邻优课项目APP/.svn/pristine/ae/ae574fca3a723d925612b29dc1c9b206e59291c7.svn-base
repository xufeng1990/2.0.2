...
export default class MyThumbShow extends Component {
    ...
    _deleteShow(showId, sectionID, rowID) {
        YLYKServices.$course.unlikeCourse(showId + '')
        .then((data) => {
    ...
    _goToPlayView(showId, is, is2) {
        if (this.state.hideNoVip == false) {
            this._alert();
             ...
             YLYKNatives.$player.openPlayerController(showId + '', false, '[]');
        }
    }
    //咨询页面
    goToQiView() {
    ...
    _getCourseList(page, callback, options) {
        ...
        InteractionManager.runAfterInteractions(() => {
            YLYKServices.$course.getCourseList({ 'is_liked': 'true', 'limit': pageLimit, 'page': page })
            .then((data) => {
                var resultData = JSON.parse(data);
                if (page === 1) {
                    that._alldata = [];
