...
public enum SkinAttrType {
    BACKGROUND("background") {
        ...
        public void apply(View view, String resName) {
            if (view instanceof ImageView) {
                Drawable divider = getResourceManager().getDrawableMipmapByName(resName);
                if (divider == null) return;
                ((ImageView) view).setImageDrawable(divider);
            }
        }
    }, PROGRESS_DRAWABLE("progress") {
        @Override
        public void apply(View view, String resName) {
            if (view instanceof SeekBar) {
                Drawable divider = getResourceManager().getDrawableByName(resName);
                if (divider == null) return;
                ((SeekBar) view).setProgressDrawable(divider);
            }
        }
    }, THUMB("thumb") {
        @Override
