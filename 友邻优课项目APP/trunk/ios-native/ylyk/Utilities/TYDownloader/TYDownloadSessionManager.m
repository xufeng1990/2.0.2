...
totalBytesExpectedToWrite:(int64_t)totalBytesExpectedToWrite
{
    ...
    downloadModel.progress.lastTimeInteval = (double)[[NSDate date] timeIntervalSince1970];

    NSTimeInterval currentTime = (double)[[NSDate date] timeIntervalSince1970];
    double lastTimeInteval = [[NSUserDefaults standardUserDefaults] doubleForKey:@"lastTimeInteval"];
    BOOL timeDistance = (double)(currentTime - lastTimeInteval) > 0.50;
    if (timeDistance) {
        [self downloadModel:downloadModel updateProgress:downloadModel.progress];
        [[NSUserDefaults standardUserDefaults] setDouble:currentTime forKey:@"lastTimeInteval"];
