...
@interface YLYKPlayerManager() <JXLoaderDelegate> {
...
- (void)addMusicProgressWithItem:(AVPlayerItem *)item {
    ...
    self.timeObserver =  [self.player addPeriodicTimeObserverForInterval:CMTimeMake(1.0, 1.0) queue:dispatch_get_main_queue() usingBlock:^(CMTime time) {
        ...
        if (current) {
            weakSelf.currentProgress = current / total;
...
//        }
//    }
    if ([self.delegate respondsToSelector:@selector(courseCacheProgress:)]) {
        [self.delegate courseCacheProgress:progress];
    }
}
