...
@interface YLYKPlayerViewController () <UITextViewDelegate,UIImagePickerControllerDelegate, UINavigationControllerDelegate,CLLocationManagerDelegate,TZImagePickerControllerDelegate,UIGestureRecognizerDelegate,playFinshhCallbackDelegate,UITableViewDelegate,UITableViewDataSource,UIWebViewDelegate>
{
    ...
    UIImage *playSetProgressDotHightlightImage;
    UIImage *playSetProgressImage;
...
- (void)viewDidLoad {
    ...
    if (!KPlayerSkinColor) {
        [[NSUserDefaults standardUserDefaults] setObject:SColor_f8fafa forKey:@"playerSkinColor"];
        [[NSUserDefaults standardUserDefaults] setObject:SColor_9a9b9c forKey:@"playerPlayListNormalColor"];
        [[NSUserDefaults standardUserDefaults] setObject:SColor_5a5a5a forKey:@"playerPlayListSelectColor"];
        [[NSUserDefaults standardUserDefaults] setObject:SColor_f8fafa forKey:@"playerFunctionTitleColor"];
        ...
        [[NSUserDefaults standardUserDefaults] setObject:KPlayerSetProgressDotHightlightImg_default forKey:@"playerSetProgressDotHighlightImg"];
        [[NSUserDefaults standardUserDefaults] setObject:KPlayerSetProgressImg_default forKey:@"playerSetProgressImg"];
        ...
        playSetProgressDotHightlightImage    = [UIImage imageNamed:KPlayerSetProgressDotHighlightImg];
        playSetProgressImage    = [UIImage imageNamed:KPlayerSetProgressImg];
    }
...
//    self.navigationController.navigationBar.alpha = 1;
    ...
    [self.navigationController.navigationBar setBarTintColor:skinColor];
    ...
    //设置播放进度条颜色
    [self.slider setMinimumTrackImage:playSetProgressImage forState:UIControlStateNormal];
    [self.slider setThumbImage:playSetProgressDotHightlightImage forState:UIControlStateHighlighted];
    [self.slider setThumbImage:playSetProgressDotImage forState:UIControlStateNormal];
    
    [playerSetSkinView setSliderProgressImg:playSetProgressImage progressDefaultImage:playSetProgressDotImage progressHighlightImage:playSetProgressDotHightlightImage];
...
//加载之前已经选择的皮肤
- (void)loadLastSelectSkinView {
    
    //加载之前保存的默认皮肤
    defaultSkinType = YLYKPlayerSkinType1;
    if ([KPlayerSkinColor isEqualToString:SColor_f8fafa]) {
        defaultSkinType = YLYKPlayerSkinType1;
        navTitleColor = [UIColor blackColor];
    } else if ([KPlayerSkinColor isEqualToString:SColor_bce0ce]) {
        defaultSkinType = YLYKPlayerSkinType2;
        navTitleColor = [UIColor blackColor];
    } else if ([KPlayerSkinColor isEqualToString:SColor_f9f2e2]) {
        defaultSkinType = YLYKPlayerSkinType3;
        navTitleColor = [UIColor blackColor];
    } else if ([KPlayerSkinColor isEqualToString:SColor_171717]) {
...
- (void)viewDidDisappear:(BOOL)animated {
    ...
    //设置导航栏显示的颜色
//    UIImage *navigationBarImg = [self imageWithColor:KColor_ffffff size:CGSizeMake(SCREEN_WIDTH, 64)];
//    [self.navigationController.navigationBar setBackgroundImage:navigationBarImg
//                                                 forBarPosition:UIBarPositionAny
//                                                     barMetrics:UIBarMetricsDefault];
//    [self.navigationController.navigationBar setShadowImage:[UIImage new]];
...
- (void) setSkin:(UIBarButtonItem*)sender {
    ...
    [playerSetSkinView getPlayerSkinCallBack:^(YLYKPlayerSkinType skinType) {
        ...
        switch (skinType) {
            ...
            {
                [[NSUserDefaults standardUserDefaults] setObject:SColor_bce0ce forKey:@"playerSkinColor"];
                ...
                [[NSUserDefaults standardUserDefaults] setObject:KPlayerSetProgressDotHightlightImg_green forKey:@"playerSetProgressDotHighlightImg"];
                [[NSUserDefaults standardUserDefaults] setObject:KPlayerSetProgressImg_green forKey:@"playerSetProgressImg"];
            ...
            {
                [[NSUserDefaults standardUserDefaults] setObject:SColor_f9f2e2 forKey:@"playerSkinColor"];
                ...
                [[NSUserDefaults standardUserDefaults] setObject:KPlayerSetProgressDotHightlightImg_yellow forKey:@"playerSetProgressDotHighlightImg"];
                [[NSUserDefaults standardUserDefaults] setObject:KPlayerSetProgressImg_yellow forKey:@"playerSetProgressImg"];
            ...
            {
                [[NSUserDefaults standardUserDefaults] setObject:SColor_171717 forKey:@"playerSkinColor"];
                ...
                [[NSUserDefaults standardUserDefaults] setObject:KPlayerSetProgressDotHightlightImg_black forKey:@"playerSetProgressDotHighlightImg"];
                [[NSUserDefaults standardUserDefaults] setObject:KPlayerSetProgressImg_black forKey:@"playerSetProgressImg"];
...
- (void)createSlider {
    ...
    self.slider=[[YLYKSlider alloc]initWithFrame:CGRectMake(45, SCREEN_HEIGHT-172 -(APP_STATUSBAR_HEIGHT - 20), SCREEN_WIDTH - 45 *2, 30)];
...
- (IBAction)showAlbumLIst:(id)sender {
    [self creatTableView];
//    NSIndexPath *scrollIndexPath = [NSIndexPath indexPathForRow:self.currentCourseIndex inSection:0];
...
BOOL loadingPause = NO;

- (void)courseCacheProgress:(CGFloat)progress {
    NSLog(@"===========%f", progress);
    self.progressView.progress = progress;
    if (progress < self.slider.value) {
//        loadingPause = NO;
        if (!loadingPause) {
             [CBLProgressHUD showLoadingHUDInView:self.view];
            loadingPause = YES;
...
- (UIView *)maskView {
    if (!_maskView) {
        ...
        maskBgview = [[UIView alloc] initWithFrame:CGRectMake(8, _maskView.frame.size.height - 225 - 8, _maskView.frame.size.width-16, 225)];
        ...
        self.fontBtn.frame = CGRectMake(25, 75, 40, 40);
        ...
        self.rateBtn.frame = CGRectMake( 25 + (40 + spaceWidth) *3 , 75, 40, 40);
        if (self.rate == 1.25) {
            [self.rateBtn setImage:[UIImage imageNamed:@"icon_speed_125"] forState:UIControlStateNormal];
        } else if(self.rate == 1.50) {
            [self.rateBtn setImage:[UIImage imageNamed:@"icon_speed_150"] forState:UIControlStateNormal];
        ...
        noteListBtn.frame = CGRectMake(25 + (40 + spaceWidth) *2, 75, 60, 40);
        ...
        self.likeBtn.frame = CGRectMake(65 + spaceWidth, 75 , 40, 40 );
