...
//        this.addOnPageChangeListener(new );
    ...
    public ExpCalendarView markDate(String date) {
        ...
        try {
            ...
            MarkedDates.getInstance().add(new DateData(thisYear, thisMonth, day));
    ...
    public ExpCalendarView markDate(ArrayList<String> dates) {
        if (dates == null) {
            return this;
        }
        ArrayList<DateData> DateDatas = new ArrayList<>();
        for (int i = 0; i < dates.size(); i++) {
            ...
            try {
                ...
                int thisMonth = calendar.get(Calendar.MONTH) +1;
                int day = calendar.get(Calendar.DAY_OF_MONTH);
                DateData dateData = new DateData(thisYear, thisMonth, day);
                DateDatas.add(dateData);
            } catch (ParseException e) {
                e.printStackTrace();
            }
        }
        MarkedDates.getInstance().addList(DateDatas);
