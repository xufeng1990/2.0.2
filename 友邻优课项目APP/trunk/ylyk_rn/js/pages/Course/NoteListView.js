...
export default class NoteListView extends Component {
    ...
    componentWillMount() {
        ...
        //获取精选心得数据
        YLYKServices.$note.getNoteList({ 'page': 1, 'is_hot': 'true', 'course_id': courseId }).then((data) => {
        ...
        // 获取课程信息
        YLYKServices.$course.getCourseById(courseId + "")
            .then((courseData) => {
                if (!courseData.id) {
    ...
    _getCourseList(page, callback, options) {
        const pageLimit = 10;
        let that = this;
        YLYKServices.$note.getNoteList({ 'page': page, 'limit': pageLimit, 'course_id': this.courseId })
            .then((data) => {
                if (page <= 1) {
                    that._alldata = [];
                };
