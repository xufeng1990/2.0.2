...
export default class FocusInformation extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = {
            isFollowee: this.props.is_followee,
            focusData: [],
    ...
    // 加载完成
    componentDidMount() {
      YLYKServices.$user.getUserById(this.props.focusId + '').then((data) => {
    ...
    _focusStateAction() {
        ...
        if (this.state.isFollowee == true) {
          YLYKServices.$followee.deleteFollowee({
                'user_id': this.props.focusId + ''
            }).then((data) => {
                DeviceEventEmitter.emit('changeFousId', rowID);
                this.setState({
                    is_followee: !this.state.isFollowee
                })
            }).catch((err) => {
                //console.log("取消错误" + err)
            })
        } else {
            YLYKServices.$followee.createFollowee({
                'user_id': this.props.focusId + ''
            }).then((data) => {
    ...
    renderHeader() {
        ...
        return (
            <StudentDetailHeader
                ...
                focusAdd={this.state.isFollowee ? '' : Icon.plus}
                focusAddText={this.state.isFollowee ? '已关注' : '关注'}
